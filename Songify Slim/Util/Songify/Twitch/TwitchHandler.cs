...
public static class TwitchHandler
{
    ...
    public static async Task<string> GetTrackIdFromInput(string input)
    {
        ...
        if (input.StartsWith("https://open.spotify.com/"))
        {
            ...
            Match match = Regex.Match(input, @"/track/([A-Za-z0-9]{22})(?=$|[/?#\s])");
    ...
    /// and pre-screened the request text (non-empty check, cooldown, user-level checks).
    /// Format-level resolution (URL parsing, track lookup) is performed inside this handler.
    /// </summary>
    ...
    /// that was already normalized and pre-screened at command ingress (non-empty check,
    /// cooldown, user-level checks). URL/ID resolution is performed inside this handler.
    /// </summary>
    /// <param name="message">Original Twitch chat message that triggered the command.</param>
    /// <param name="requestInput">Normalized request text extracted at ingress.</param>
    /// <param name="cmdParams">Resolved user and command execution context.</param>
